...
def start_image_check(simulation_id_str=None, project_name='Simulation Project', project_type='1'):
    img_fname_list = glob.glob('%s/*.JPG' % Config.DIRECTORY_IMAGE_CHECK)

    # 현재 프로젝트 설정
    ldm = Livedronemap(Config.LDM_ADDRESS)
    drone_project_id = ldm.create_project(project_name=project_name, project_type=project_type)  # project_type 0: 실프로젝트, 1: 시뮬레이션
...
if __name__ == '__main__':
    start_image_check(project_name='Simulation Project [YONGSAN]', project_type='0')
